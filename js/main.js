$(function(){$('.j-tooltip').tooltip();var gem_install_message_pre="\
...
\n7 gems installed\
\n    ";var prompt="[[b;#d33682;]root]@[[b;#6c71c4;]wedding] ~$ ";var days_left=Math.round((new Date('2013 11 29')-new Date())/(1000*60*60*24));var wedding_url="http://goo.gl/4tqfMs";var rsvp_url="https://twitter.com/jaipandya";var venue_address="\
...
\n  ";var gem_list_empty='\n*** LOCAL GEMS ***\n';var gem_list_full='\n*** LOCAL GEMS ***\n\naddressable (2.3.5)\nlaunchy (2.3.0)\nrequire_all (1.3.2)\nthor (0.18.1)\nartii (2.0.3)\nrainbow (1.1.4)\nwedding (0.0.1)\n';var print_gem='RubyGems is a sophisticated package manager for Ruby.  This is a\nbasic help message containing pointers to more information.\n\n  Usage:\n    gem -v\n    gem command [arguments...] [options...]\n\n  Examples:\n    gem install rake\n    gem list --local\n    gem build package.gemspec\n    gem help install\n\n  Further help:\n    gem help commands            list all \'gem\' commands\n    gem help examples            show some examples of usage\n    gem help platforms           show information about platforms\n    gem help <COMMAND>           show help on COMMAND\n                                   (e.g. \'gem help install\')\n    gem server                   present a web page at\n                                 http://localhost:8808/\n                                 with info about installed gems\n  Further information:\n    http://rubygems.rubyforge.org';function print_slowly(term,paragraph,callback){var foo,i,lines;lines=paragraph.split("\n");term.pause();i=0;foo=function(lines){return setTimeout((function(){if(i<lines.length-1){term.echo(lines[i]);i++;return foo(lines);}else{term.resume();return callback();}}),1000);};return foo(lines);};function require_command(command_regex,terminal_history){var executed=false;$.each(terminal_history,function(index,value){if(command_regex.test(value)){executed=true}});return executed;}
var gem_install_regex=/gem +install +wedding */ig;function gem(inputs,term){if(!inputs[1]){term.echo(print_gem);}else if(inputs[1]==='install'&&inputs[2]==='wedding'){print_slowly(term,gem_install_message_pre,function(){term.echo(gem_install_message_post)});}else if(inputs[1]==='-v'){term.echo('1.3.6');}else if(inputs[1]=='list'){term.echo(gem_list_full);}else{term.echo(inputs.join(" ")+" is not a valid command")}}
function wedding(inputs,term){if(!inputs[1]){term.echo(wedding_help);}else if(inputs[1]==="bride"){term.echo(bride);}else if(inputs[1]==="groom"){term.echo(groom)}else if(inputs[1]==="invitation"){term.echo("[[;#b58900;]"+ganesha+"]");term.echo(invitation_pre);term.pause();setTimeout(function(){term.resume();term.echo(jai_weds_prerita);term.pause();setTimeout(function(){term.resume();term.echo(invitation_post);},400)},1500)}else if(inputs[1]==="location"){term.echo(venue_address);term.echo(wedding_url);term.push(function(command,term){if(/y(es){0,1}/.test(command)){window.open(wedding_url,'_blank');}
term.pop();},{prompt:'Do you want to open this link in the browser? (yes/no) ',greetings:null});}else if(inputs[1]==="rsvp"){term.echo(rsvp)
term.push(function(command,term){window.open(rsvp_url,'_blank');term.pop();},{prompt:'alternatively send me a tweet in a new window when you press [[b;#859900;]enter (↩)]',greetings:null});}else{term.error(inputs.join(" ")+" is not a valid command")}}
function interpreter(input,term){var command,inputs;inputs=input.split(/ +/)
command=inputs[0];if(command==="gem"){gem(inputs,term);}else if(command==="wedding"||command==="help"){window.terminal=term;if(require_command(gem_install_regex,term.history().data())){wedding(inputs,term);}else{term.error('Please install the gem first by executing\ngem install wedding');}}else if(/(cd)|(ls)|(cat)/.test(command)){bash(inputs,term);}else if(input==="ruby -v"){term.echo("1.9.3");}else if(/which +wedding/.test(input)){if(require_command(gem_install_regex,term.history().data())){term.echo("/usr/bin/wedding");}}else if(/whoami/.test(input)){term.echo("root");}else if(command.length===0){}else{term.error(command+" is not a valid command");}}
function bash(inputs,term){var argument,echo,insert;echo=term.echo;insert=term.insert;if(!inputs[1]){return console.log("none");}else{argument=inputs[1];if(/^\.\./.test(argument)){return echo("-bash: cd: "+argument+": Permission denied");}else{return echo("-bash: cd: "+argument+": No such file or directory");}}};$('#terminal').terminal(interpreter,{prompt:prompt,name:'wedding',greetings:greetings,height:600,onInit:function(term){term.insert("gem install wedding");term.history().clear();},completion:function(term,string,callback){callback(['gem install wedding','wedding invitation','wedding location','wedding groom','wedding rsvp','wedding bride']);},tabcompletion:true});});